locals {
  ...
  private_zone_enabled       = var.private_zone_enabled && module.this.enabled ? 1 : 0
}
...
resource "aws_route53_zone" "default" {
  ...
  dynamic "vpc" {
    for_each = local.private_zone_enabled > 0 ? [true] : []

    content {
      vpc_id     = var.vpc_id
      vpc_region = data.aws_region.default.name
    }
  }
...
resource "aws_route53_record" "soa" {
  count           = local.enabled > 0 && local.private_zone_enabled < 1 ? 1 : 0
